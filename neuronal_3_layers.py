...
class NeuralNet:
    ...
    def __init__(self, batch_size, nb_classes, file):
        ...
        self.layer2_weights = np.random.rand(5,5)
        self.layer3_weights = np.random.rand(5, 1) # Matrice de poids
    ...
    def training(self, list_input, list_output, number_epoch):
        ...
        for i in range(number_epoch):
            ...
            for j in range(len(batch_input)):
                ...
                layer1_weights_output, layer2_weights_output, layer3_weights_output = self.testing(batch_input[j])

                # On calcule l'erreur et on rétropopage en réajustant les poids
                # D'abord l'erreur et le delta de la dernière couche
                layer3_error = batch_output[j] - layer3_weights_output
                layer3_delta = layer3_error*self.__sigmoid_derivative(layer3_weights_output)
                layer3_adjustment = layer2_weights_output.transpose().dot(layer3_delta)
                self.layer3_weights += layer3_adjustment*self.taux_apprentissage
                # Puis la couche précédente
                layer2_error = layer3_delta.dot(self.layer3_weights.transpose())
    ...
    def testing(self, input):
        ...
        layer3_output = self.__sigmoid(dot(layer2_output, self.layer3_weights))
...
if __name__ == "__main__":
    ...
    for i in range(len(testing_data_input)):
        ...
        initial_weight, mid_weight, output = nn.testing(array(testing_data_input[i]))
